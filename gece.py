    ...
    async for usr in client.iter_participants(event.chat_id):
     ...
     await event.reply(f"{ad} {startmesaj}", buttons=(
                     ...
                     [Button.url('📣 ASO Söhbət Qrupu', f'https://t.me/{WerabliAnlar}')],
                      [Button.url('📣 ASO Rəsmi', f'https://t.me/{ASOresmi}')],
...
async def mentionall(event):
  ...
  if mode == "text_on_cmd":
    await client.send_message(event.chat_id, "❄️ Tağ Başladı\n⏱️ İnterval - 2 saniyə",
                    buttons=(
                      [
                      Button.url('📣 Support', f'https://t.me/WerabliAnlar{}')
                      ]
                    )
                  ) 
    ...
    async for usr in client.iter_participants(event.chat_id):
      ...
      if event.chat_id not in gece_tag:
        await event.respond("⛔ Tağ Prosesi Dayandırıldı @ASOresmi",
                    buttons=(
                      [
                       Button.url('📣 Support', f'https://t.me/{WerabliAnlar}')
                      ]
...
async def tektag(event):
  ...
  if mode == "text_on_cmd":
    ...
    async for usr in client.iter_participants(event.chat_id):
      ...
      usrtxt += f"[{usr.first_name}](tg://user?id={usr.id}) "
      if event.chat_id not in gece_tag:
        await event.respond("⛔ Tek tek Tağ Prosesi Dayandırıldı @WerabliAnlar",
                    buttons=(
                      [
                       Button.url('📣 Support', f'https://t.me/{WerabliAnlar}')
                      ]
...
print(">> Bot Artıq İşləyir 🚀 @ismiyev95 məlumat ala bilirsən <<")
